...
║  SCRIPTS EXPERT                  ║
║                                  ║
╚══════════════════════════════════╝`, "Sending heartbeat each 30s - Updating point each 5 minutes"];
...
let wallets = [];
try {
  wallets = fs.readFileSync("wallets.txt", "utf8").trim().split(/\s+/).filter(Boolean);
} catch (err) {
  console.error("Error reading wallets.txt:", err.message);
  process.exit(1);
}

let proxies = [];
try {
  proxies = fs.readFileSync("proxy.txt", "utf8").trim().split(/\s+/).filter(Boolean);
} catch (error) {
...
async function getOrCreateWalletData(address, agent) {
  ...
  if (!dataStore[address].token || (dataStore[address].token && isTokenExpired(dataStore[address].token))) {
    ...
    try {
      ...
      if (!claimed) {
        ...
        if (claimRewardResponse.data.status === "SUCCESS") {
          console.log(`\x1b[33m[${index + 1}]\x1b[0m Wallet \x1b[36m${address}\x1b[0m, ` + `AccountID \x1b[36m${accountIDs[address]}\x1b[0m \x1b[32mClaimed daily reward successfully!\x1b[0m`);
        }
      }
      return;
    } catch (error) {
...
function connectWebSocket({ token, workerID, id, address }, index, useProxy) {
  ...
  function sendHeartbeat() {
    ...
    // console.log(`\x1b[33m[${index + 1}]\x1b[0m AccountID \x1b[36m${accountIDs[address]}\x1b[0m: ` + `Sending heartbeat for workerID: \x1b[33m${workerID}\x1b[0m, Proxy: \x1b[36m${proxyText}\x1b[0m`);
    ws.send(JSON.stringify(heartbeatMessage));
  }

  ws.on("open", () => {
    console.log(
      `\x1b[33m[${index + 1}]\x1b[0m AccountID \x1b[36m${accountIDs[address]}\x1b[0m: ` + `Connected to WebSocket for workerID: \x1b[33m${workerID}\x1b[0m, Proxy: \x1b[36m${proxyText}\x1b[0m`
    );
  ...
  ws.on("message", (data) => {
    console.log(`\x1b[33m[${index + 1}]\x1b[0m AccountID \x1b[36m${accountIDs[address]}\x1b[0m: ` + `Received for workerID \x1b[33m${workerID}\x1b[0m: ${data}, Proxy: \x1b[36m${proxyText}\x1b[0m`);
  });

  ws.on("error", (err) => {
    console.error(`\x1b[33m[${index + 1}]\x1b[0m AccountID \x1b[36m${accountIDs[address]}\x1b[0m: ` + `WebSocket error for workerID \x1b[33m${workerID}\x1b[0m:`, err);
  });

  ws.on("close", () => {
    console.log(
      `\x1b[33m[${index + 1}]\x1b[0m AccountID \x1b[36m${accountIDs[address]}\x1b[0m: ` + `WebSocket connection closed for workerID \x1b[33m${workerID}\x1b[0m, Proxy: \x1b[36m${proxyText}\x1b[0m`
    );
    clearInterval(heartbeatInterval);

    setTimeout(() => {
      console.log(
        `\x1b[33m[${index + 1}]\x1b[0m AccountID \x1b[36m${accountIDs[address]}\x1b[0m: ` + `Reconnecting WebSocket for workerID: \x1b[33m${workerID}\x1b[0m, Proxy: \x1b[36m${proxyText}\x1b[0m`
      );
  ...
  for (let tierId = 1; tierId <= 8; tierId++) {
    try {
      ...
      if (response.data.status === "SUCCESS" && response.data.data === true) {
        console.log(`\x1b[33m[${index + 1}]\x1b[0m Wallet \x1b[36m${address}\x1b[0m: ` + `Successfully claimed medal for tier \x1b[32m${tierId}\x1b[0m`);
      }
    } catch (error) {}
  }
}
...
async function processRequests(useProxy) {
  const promises = wallets.map(async (address, index) => {
    ...
    await Promise.all([claimMedals(record.token, address, index, useProxy), checkAndClaimReward(record.token, address, index, useProxy), getAccountDetails(record.token, address, index, useProxy)]);
